...
class CartPoleEnv2(gym.Env):
	...
	reward = -n*(q[0]*np.power(x,2) + q[1]*np.power(x_dot,2) + q[1]*np.power(theta,2) + q[3]*np.power(theta_dot,2))
#	reward = -n*(q[0]*x*x + q[1]*x_dot*x_dot + q[2]*theta*theta + q[3]*theta_dot*theta_dot + r*force*force)
	#print x, theta, reward
