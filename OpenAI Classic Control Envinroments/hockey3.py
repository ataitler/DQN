...
class HockeyEnv3(gym.Env):
	...
	def _reset(self):
		...
		low = np.array([-self.max_x + 1. + self.agentR,  0.0, 0, 0, round(-self.puckR,1),         round(-self.max_y + self.puckR + 4.0,1), 0, 0])
		high = np.array([-self.max_x + 1. + self.agentR, 0.0, 0, 0, round(-self.max_x/3.5*2.0,1), round(self.max_y - self.puckR - 4.0,1),  0, 0])
	...
	def calcTrajectoryProperties(self,px,py,pVx,pVy, target_point):
		...
		#print px,py, theta*180/np.pi, np.sin(theta),pVx,pVy, V
	...
	def calcReward(self, goal_point, speed):
		...
		#print x, speed, features, temp_reward
		return temp_reward
