...
# global definitions
OUT_DIR = "hockey_extended_val_big/"
T = -0.5
...
VALIDATION_FILE = 'validation_buffer'
LOG_FILE = 'logs'
DISPLAY = False
...
buf = R_val.LoadBuffer(VALIDATION_FILE)
if buf:
	populated = R_val.GetOccupency()
	print("Validation buffer loaded from disk, size: " + str(populated))
else:
	print("No validation buffer found!")
...
for episode_i in xrange(1,EPISODES+1):
	...
	iterations = np.ceil(1.0 * R_val.GetOccupency() / MINI_BATCH).astype(int)
	valerror = []
	for i in xrange(iterations):
		s_val, a_val, r_val, stag_val, terminal_val, num = R_val.GetMiniBatch(MINI_BATCH, i)
		if num == 0:
			continue
		Y = Q.evaluate(sess, s_val)
		Q_next_arg = Q.evaluate(sess, stag_val)
		Q_next_argmax = np.argmax(Q_next_arg,1)
		Q_next_target = Q_target.evaluate(sess, stag_val)

		a_batch = a_val.astype(int)
		for i in range(num):
			Y[i,a_val[i,0]] = r_val[i,0] + GAMMA*Q_next_target[i,Q_next_argmax[i]] * (1-terminal_val[i])

		valerror.append(Q.error(sess, s_val, Y))
	if len(valerror) == 0:
		valerror = 0
	else:
		valerror = sum(valerror)/len(valerror)
	L.AddRecord('validation_error', valerror)
...
valerror = L.GetLogByName('validation_error')
...
plt.figure(5)
plt.plot(t, error, 'b', t, valerror, 'r')
plt.xlabel('Episodes')
plt.ylabel('error')
